...
/* Create procedure */
USE `moneysmart`; -- if you have error 1046, tell mysql which database to use
DROP PROCEDURE IF EXISTS proc_order; -- drop the procedure if it already existed

DELIMITER $$ -- define delimiter from ; to $$
...
BEGIN
    ...
    DECLARE v_finished INT DEFAULT 0; -- created to escape the roop
...
GROUP BY datediff
ORDER BY Order_Count DESC -- show data from the most frequent date interval
LIMIT 5;  -- show top 5 of date interval only   
